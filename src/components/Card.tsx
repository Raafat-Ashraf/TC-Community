...
export default function Card({
  ...
  const content = (
    <div
      className={clsx(
        "group relative flex h-full flex-col overflow-hidden rounded-2xl border border-cream-200 bg-white shadow-sm transition-all duration-300",
    ...
    >
      {image && (
        <div className="relative h-40 w-full overflow-hidden">
          ...
          />
          {comingSoon && (
            <div className="absolute inset-0 bg-navy-950/40" aria-hidden="true" />
          )}
        </div>
      )}
